...
function showTab(tab,el){
  ...
  ["songsTab","recentTab","favTab"].forEach(id=>{
...
function renderSongs(){
  let songs = getSongs();   // 🔥 always fresh data
...
function renderRecent(){
  let songs = getSongs();
  let r = JSON.parse(localStorage.recent || "[]");
  recentTab.innerHTML = r.length
    ? r.map(i=>`<div class="song-item" onclick="openLyrics(${i})">${songs[i].title}</div>`).join("")
    : "<p>No recent songs</p>";
}
...
function renderFav(){
  ...
  favTab.innerHTML = f.length
    ? f.map(i=>`<div class="song-item" onclick="openLyrics(${i})">${songs[i].title}</div>`).join("")
    : "<p>No favorites</p>";
}
...
function searchSongs(){
  ...
  document.querySelectorAll(".song-item span").forEach(s=>{
    s.parentElement.style.display =
      s.textContent.toLowerCase().includes(q) ? "block" : "none";
  });
}
